...
import ${packageName}.infra.mapper.${upperDataKey}Mapper;
import ${packageName}.web.dto.${dataKey}.${upperDataKey}QueryRequest;
import ${packageName}.infra.persistence.entity.${upperDataKey};
import ${packageName}.infra.persistence.entity.${upperDataKey}Favour;
import ${packageName}.infra.persistence.entity.${upperDataKey}Thumb;
import ${packageName}.infra.persistence.entity.User;
import ${packageName}.web.vo.${upperDataKey}VO;
import ${packageName}.web.vo.UserVO;
import ${packageName}.service.${upperDataKey}Service;
import ${packageName}.service.UserService;
import ${packageName}.common.util.SqlUtils;
...
public class ${upperDataKey}ServiceImpl extends ServiceImpl<${upperDataKey}Mapper, ${upperDataKey}> implements ${upperDataKey}Service {
     ...
     */
    @Override
    public QueryWrapper getQueryWrapper(${upperDataKey}QueryRequest ${dataKey}QueryRequest) {
    ...
    public ${upperDataKey}VO get${upperDataKey}VO(${upperDataKey} ${dataKey}, HttpServletRequest request) {
        ...
        if (loginUser != null) {
            // 获取点赞
            QueryWrapper<${upperDataKey}Thumb> ${dataKey}ThumbQueryWrapper = QueryWrapper.create();
            ${dataKey}ThumbQueryWrapper.in("${dataKey}Id", ${dataKey}Id);
            ${dataKey}ThumbQueryWrapper.eq("userId", loginUser.getId());
            ${upperDataKey}Thumb ${dataKey}Thumb = ${dataKey}ThumbMapper.selectOneByQuery(${dataKey}ThumbQueryWrapper);
            ${dataKey}VO.setHasThumb(${dataKey}Thumb != null);
            // 获取收藏
            QueryWrapper<${upperDataKey}Favour> ${dataKey}FavourQueryWrapper = QueryWrapper.create();
            ${dataKey}FavourQueryWrapper.in("${dataKey}Id", ${dataKey}Id);
            ${dataKey}FavourQueryWrapper.eq("userId", loginUser.getId());
            ${upperDataKey}Favour ${dataKey}Favour = ${dataKey}FavourMapper.selectOneByQuery(${dataKey}FavourQueryWrapper);
    ...
    public Page<${upperDataKey}VO> get${upperDataKey}VOPage(Page<${upperDataKey}> ${dataKey}Page, HttpServletRequest request) {
        ...
        Page<${upperDataKey}VO> ${dataKey}VOPage = new Page<>(${dataKey}Page.getPageNumber(), ${dataKey}Page.getPageSize(), ${dataKey}Page.getTotalRow());
        ...
        if (loginUser != null) {
            ...
            ${dataKey}ThumbQueryWrapper.eq("userId", loginUser.getId());
            List<${upperDataKey}Thumb> ${dataKey}${upperDataKey}ThumbList = ${dataKey}ThumbMapper.selectListByQuery(${dataKey}ThumbQueryWrapper);
            ...
            ${dataKey}FavourQueryWrapper.eq("userId", loginUser.getId());
            List<${upperDataKey}Favour> ${dataKey}FavourList = ${dataKey}FavourMapper.selectListByQuery(${dataKey}FavourQueryWrapper);
