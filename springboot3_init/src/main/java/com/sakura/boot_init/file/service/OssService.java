...
public class OssService {
    ...
    public String uploadFile(MultipartFile file) {
        ...
        if (originalFilename == null || originalFilename.isEmpty()) {
            throw new BusinessException(ErrorCode.PARAMS_ERROR, "文件名为空，上传失败");
        }
    ...
    private String buildAccessUrl(String objectName) {
        if (StringUtils.isNotBlank(ossProperties.getDomain())) {
            return StringUtils.removeEnd(ossProperties.getDomain(), "/") + "/" + objectName;
        }
        String protocol = "N".equalsIgnoreCase(ossProperties.getIsHttps()) ? "http" : "https";
        return String.format("%s://%s.%s/%s", protocol, ossProperties.getBucketName(), ossProperties.getEndpoint(),
                objectName);
    }
}
